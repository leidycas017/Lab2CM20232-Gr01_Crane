        ...
        frontLayerContent = {
            HorizontalPager(state = pagerState) { page ->
                ...
                when (craneScreenValues[page]) {
                    ...
                    CraneScreen.Sleep -> {
                        LazyColumn {
                            items(destination) { destination: DestinationItem ->
...
fun DestinationCard(destination: DestinationItem){
    ...
    val image = rememberImagePainter(data = destination.imageUrl)
    ...
    Card(
        ...
        Column(modifier = Modifier.padding(10.dp)) {
            Text(text = destination.city, fontWeight = FontWeight.Bold)
